...
# How many days a task stays visible in the "Done" column after being
# marked done, before it's hidden from the board and moved to the archive.
...
async def set_notifications_enabled(telegram_id: int, enabled: bool) -> dict:
    """Toggle morning digest notifications for a user."""
    db = await get_db()
    try:
        await db.execute(
...
async def get_team_tasks(team_id: int, status: str = None,
                         ...
                         tag_id: int = None, include_archived: bool = False) -> list:
    """
    By default, "done" tasks older than DONE_ARCHIVE_DAYS (based on when they
    were moved to done — status_changed_at, falling back to updated_at for
    legacy rows) are excluded from the result to keep the board tidy.
    Pass include_archived=True to get ONLY those archived done tasks instead
    (used by the separate archive view).
    """
    db = await get_db()
    try:
        query = """
        ...
        done_since = "COALESCE(t.status_changed_at, REPLACE(t.updated_at, 'T', ' '))"
        if include_archived:
            query += f" AND t.status = 'done' AND {done_since} < datetime('now', ?)"
            params.append(f'-{DONE_ARCHIVE_DAYS} days')
        else:
            query += f" AND (t.status != 'done' OR {done_since} >= datetime('now', ?))"
            params.append(f'-{DONE_ARCHIVE_DAYS} days')
...
async def get_team_stats(team_id: int, period: str = 'all') -> dict:
    from datetime import date, timedelta
    ...
    if period == 'week':
        today = date.today()
        monday = today - timedelta(days=today.weekday())
        date_filter = f"AND date(t.created_at) >= '{monday.isoformat()}'"
    elif period == 'month':
        today = date.today()
        date_filter = f"AND strftime('%Y-%m', t.created_at) = '{today.strftime('%Y-%m')}'"
    elif period == 'quarter':
        today = date.today()
        quarter_start = date(today.year, ((today.month - 1) // 3) * 3 + 1, 1)
        date_filter = f"AND date(t.created_at) >= '{quarter_start.isoformat()}'"

    db = await get_db()
    try:
        cursor = await db.execute(
            f"SELECT COUNT(*) as total FROM tasks WHERE team_id=? {date_filter}", (team_id,)
        )
        total = (await cursor.fetchone())['total']

        cursor = await db.execute(
            f"SELECT status, COUNT(*) as cnt FROM tasks WHERE team_id=? {date_filter} GROUP BY status",
            (team_id,)
        )
        by_status = {r['status']: r['cnt'] for r in await cursor.fetchall()}

        cursor = await db.execute(
            f"SELECT priority, COUNT(*) as cnt FROM tasks WHERE team_id=? {date_filter} GROUP BY priority",
            (team_id,)
        )
        ...
        cursor = await db.execute(f"""
            ...
            FROM users u
            JOIN team_members tm ON u.id = tm.user_id
            LEFT JOIN tasks t ON t.assignee_id = u.id AND t.team_id = ? {date_filter}
            WHERE tm.team_id = ?
            GROUP BY u.id
        """, (team_id, team_id))
        ...
        cursor = await db.execute(f"""
            ...
            FROM tags tg
            LEFT JOIN tasks t ON t.tag_id = tg.id AND t.team_id = ? {date_filter}
            WHERE tg.team_id = ?
            GROUP BY tg.id
        """, (team_id, team_id))
...
async def get_checklist(task_id: int) -> list:
    db = await get_db()
    try:
        cursor = await db.execute(
...
async def add_checklist_item(task_id: int, text: str, position: int = 0) -> dict:
    db = await get_db()
    try:
        await db.execute(
        ...
        await db.commit()
        item_id = (await (await db.execute("SELECT last_insert_rowid()")).fetchone())[0]
        cursor = await db.execute("SELECT * FROM checklist_items WHERE id=?", (item_id,))
        return dict(await cursor.fetchone())
    finally:
        await db.close()


async def toggle_checklist_item(item_id: int, is_done: bool) -> dict:
    db = await get_db()
    try:
        await db.execute(
            "UPDATE checklist_items SET is_done=? WHERE id=?",
...
async def delete_checklist_item(item_id: int) -> bool:
    db = await get_db()
    try:
        cursor = await db.execute("DELETE FROM checklist_items WHERE id=?", (item_id,))
        await db.commit()
        return cursor.rowcount > 0
    finally:
        await db.close()
...
async def update_checklist_item(item_id: int, text: str) -> dict:
    db = await get_db()
    try:
        await db.execute("UPDATE checklist_items SET text=? WHERE id=?", (text, item_id))
        await db.commit()
        cursor = await db.execute("SELECT * FROM checklist_items WHERE id=?", (item_id,))
        return dict(await cursor.fetchone())
    finally:
...
async def remove_team_member(team_id: int, user_id: int) -> bool:
    """Remove a member from a team (owner cannot be removed)."""
    db = await get_db()
    try:
...
async def update_tag(tag_id: int, team_id: int, name: str, color: str) -> Optional[dict]:
    db = await get_db()
    try:
        await db.execute(
            "UPDATE tags SET name=?, color=? WHERE id=? AND team_id=?",
            (name, color, tag_id, team_id)
        )
        await db.commit()
