...
async def fsm_got_priority(callback: types.CallbackQuery, state: FSMContext):
    ...
    if data.get('assignee_id') and data['assignee_id'] != data['user_id']:
        await notify_task_assigned(task, (await db.get_user_by_telegram_id(
            (await db.get_or_create_user(telegram_id=callback.from_user.id)).get('telegram_id', 0)
        ) or {}).get('first_name') or callback.from_user.first_name or 'Кто-то')
...
async def check_deadlines():
    ...
    while True:
        try:
            # Get all tasks with deadlines that aren't done
            ...
            try:
                ...
                for task in due_24h:
                    ...
                    await send_notification(
                        task_dict['assignee_telegram_id'],
                        f"🟡 {hbold('Дедлайн завтра, хуярь брат!')}:\n\n"
                        f"📋 {hbold(task_dict['title'])}\n"
                        f"⏰ Дедлайн: {_fmt_deadline(task_dict['deadline'])}\n"
                        f"Приоритет: {priority_emoji(task_dict['priority'])} {task_dict['priority']}\n\n"
                        f"Осталось меньше 24 часов — не тяни 💪",
                        task_dict['id']
                    )
                    await db_conn.execute(
                ...
                for task in due_1h:
                    ...
                    await send_notification(
                        task_dict['assignee_telegram_id'],
                        f"🔴 {hbold('ЧЕРЕЗ ЧАС ДЕДЛАЙН, хуярь брат!')}:\n\n"
                        f"📋 {hbold(task_dict['title'])}\n"
                        f"⏰ Дедлайн: {_fmt_deadline(task_dict['deadline'])}\n"
                        f"Приоритет: {priority_emoji(task_dict['priority'])} {task_dict['priority']}\n\n"
                        f"Последний шанс — давай! 🔥",
                        task_dict['id']
                    )
                    await db_conn.execute(
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        types.BotCommand(command="newtask", description="Создать задачу (диалог)"),
