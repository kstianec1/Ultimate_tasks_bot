...
async def cmd_help(message: types.Message):
    text = (
        ...
        f"/kick &lt;@username&gt; — удалить участника\n"
    ...
    if text not in ('нет', 'no', 'skip', '-', 'пропустить'):
        ...
        if dt_match:
            ...
            try:
                ...
                dt_msk = datetime(int(y), int(mo), int(d), int(h), int(mi))
                # Convert to UTC
                dt_utc = dt_msk - timedelta(hours=3)
                deadline = dt_utc.strftime('%Y-%m-%d %H:%M')
            except ValueError:
                await message.answer(
            ...
            try:
                from datetime import datetime, timezone, timedelta
                # Parse as Moscow time (UTC+3) at 23:59
                dt_msk = datetime(int(y), int(mo), int(d), 23, 59)
                # Convert to UTC
                dt_utc = dt_msk - timedelta(hours=3)
                deadline = dt_utc.strftime('%Y-%m-%d %H:%M')
            except ValueError:
...
async def cmd_kick(message: types.Message):
    """Remove a member from team. Usage: /kick @username"""
    user = await ensure_user(message)
    teams = await db.get_user_teams(user['id'])

    # Only owners can kick
    owner_teams = [t for t in teams if t['role'] == 'owner']
    if not owner_teams:
        await message.answer("❌ Только владелец команды может удалять участников.")
        return
    ...
    if len(args) < 2:
        await message.answer(
            ...
            "Пример: /kick @ivan\n\n"
            "Удаляет участника из твоей команды."
        )
        return
    ...
    # Try to remove from all owner teams
    removed = False
    for team in owner_teams:
        result = await db.remove_team_member(team['id'], target['id'])
        if result:
            removed = True
            await message.answer(
                f"✅ {hbold(target.get('first_name') or username)} удалён из команды {hbold(team['name'])}."
            )

    if not removed:
        await message.answer(f"❌ @{username} не является участником твоих команд или является владельцем.")
...
async def check_deadlines():
    ...
    while True:
        try:
            ...
            weekday_msk = now_msk.weekday()  # 0=Monday, 6=Sunday
            ...
            # ── Morning digest at 10:00 MSK on weekdays only (Mon-Fri) ──
            if hour_msk == 10 and last_digest_date != today_msk and weekday_msk < 5:  # 0-4 = Mon-Fri
                last_digest_date = today_msk
                await _send_morning_digest()

            # Skip weekend notifications entirely
            if weekday_msk >= 5:  # 5=Saturday, 6=Sunday
            ...
            # Only send morning digest at 10:00, no other deadline notifications
            # Remove all other notification logic
            await asyncio.sleep(1800)
            continue
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        types.BotCommand(command="kick", description="Удалить участника из команды"),
