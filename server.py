...
def validate_init_data(init_data: str) -> dict | None:
    ...
    try:
        if not init_data or len(init_data) < 10:
            logger.warning(f"Empty or too short init_data: {len(init_data) if init_data else 0} chars")
            return None
        ...
        if computed_hash != check_hash:
            ...
            logger.warning(f"Hash mismatch. Expected: {check_hash[:8]}..., Got: {computed_hash[:8]}...")
            logger.warning(f"Data check string length: {len(data_check_string)}")
            
            # Check if auth_date is too old (more than 24 hours)
            ...
            if auth_date_str:
                try:
                    ...
                    if current_time - auth_date > 86400:  # 24 hours
                        logger.warning(f"Auth date too old: {current_time - auth_date} seconds difference")
                except Exception as e:
                    logger.warning(f"Failed to parse auth_date: {e}")
...
async def auth_middleware(request, handler):
    ...
    # Development mode: allow bypass with debug user (only if DEBUG=True)
    DEBUG_MODE = os.getenv("DEBUG", "").lower() in ("1", "true", "yes")
    if DEBUG_MODE and not init_data and request.headers.get("X-Debug-User-Id"):
        try:
            debug_user_id = int(request.headers.get("X-Debug-User-Id"))
            # Create or get debug user
            ...
            request["user"] = user
            logger.warning(f"DEBUG MODE: Using debug user {debug_user_id} for {request.path}")
            return await handler(request)
        except Exception as e:
...
async def get_tasks(request):
    ...
    if tag_id:
        tag_id = int(tag_id)
    # archived=1 returns ONLY the done tasks hidden from the normal board
    # (done for more than DONE_ARCHIVE_DAYS days) — used by the archive view.
...
async def update_tag(request):
    ...
    if not await db.is_team_member(tag["team_id"], user["id"]):
        return web.json_response({"error": "Forbidden"}, status=403)
    data = await request.json()
    updated = await db.update_tag(tag_id, tag["team_id"],
                                   data.get("name", tag["name"]),
                                   data.get("color", tag["color"]))
    return web.json_response(updated)


# ── Checklist ──

@routes.get("/api/tasks/{task_id}/checklist")
async def get_checklist(request):
    user = request["user"]
    task_id = int(request.match_info["task_id"])
    task = await db.get_task(task_id)
    if not task or not await db.is_team_member(task["team_id"], user["id"]):
        return web.json_response({"error": "Forbidden"}, status=403)
    items = await db.get_checklist(task_id)
    return web.json_response(items)


@routes.post("/api/tasks/{task_id}/checklist")
async def add_checklist_item(request):
    user = request["user"]
    task_id = int(request.match_info["task_id"])
    task = await db.get_task(task_id)
    if not task or not await db.is_team_member(task["team_id"], user["id"]):
    ...
    item = await db.add_checklist_item(task_id, text, data.get("position", 0))
    return web.json_response(item, status=201)


@routes.patch("/api/checklist/{item_id}")
async def update_checklist_item(request):
    user = request["user"]
    item_id = int(request.match_info["item_id"])
    data = await request.json()
    if "is_done" in data:
        item = await db.toggle_checklist_item(item_id, data["is_done"])
    elif "text" in data:
        item = await db.update_checklist_item(item_id, data["text"])
    else:
        return web.json_response({"error": "Nothing to update"}, status=400)
    return web.json_response(item)


@routes.delete("/api/checklist/{item_id}")
async def delete_checklist_item(request):
    user = request["user"]
    item_id = int(request.match_info["item_id"])
    await db.delete_checklist_item(item_id)
    return web.json_response({"ok": True})


# ── Team member management ──

@routes.delete("/api/teams/{team_id}/members/{user_id}")
async def remove_member(request):
    ...
    if not row or row["role"] not in ("owner", "admin"):
        return web.json_response({"error": "Forbidden"}, status=403)
    removed = await db.remove_team_member(team_id, target_user_id)
    if not removed:
        return web.json_response({"error": "Cannot remove owner or member not found"}, status=400)
    return web.json_response({"ok": True})
...
@routes.get("/api/me/settings")
async def get_my_settings(request):
    user = request["user"]
...
async def update_my_settings(request):
    user = request["user"]
    data = await request.json()
    if "notifications_enabled" in data:
        updated = await db.set_notifications_enabled(
            user["telegram_id"], bool(data["notifications_enabled"])
        )
        return web.json_response({
            "notifications_enabled": bool(updated.get("notifications_enabled", 1))
        })
    return web.json_response({"error": "Nothing to update"}, status=400)
...
async def get_stats(request):
    ...
    stats = await db.get_team_stats(team_id, period=period)
...
async def serve_webapp(request):
    ...
    if not os.path.exists(index_path):
        logger.error(f"index.html not found at {index_path}, WEBAPP_DIR={WEBAPP_DIR}")
        return web.Response(text="App loading...", content_type="text/html")
